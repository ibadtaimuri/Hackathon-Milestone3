...
function generateResume() {
    ...
    for (var i = 1; i <= educationCount; i++) {
        ...
        if (degree && institution && year) {
            educationInfo.innerHTML += "\n                <p><strong>".concat(degree, "</strong>, ").concat(institution, " (").concat(year, ")</p>\n            ");
        }
    }
    ...
    for (var i = 1; i <= workCount; i++) {
        ...
        if (position && company && years && details) {
            workExperienceInfo.innerHTML += "\n                <p><strong>".concat(position, "</strong> at ").concat(company, " (").concat(years, ")</p>\n                <p>").concat(details, "</p>\n            ");
        }
    }
    // Populating the Skills Section
    skillsInfo.innerHTML = "<h2>Skills</h2><p>".concat(skills.split(',').map(function (skill) { return "<span>".concat(skill.trim(), "</span>"); }).join(', '), "</p>");
}
